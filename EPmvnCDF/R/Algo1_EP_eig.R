#' FASt CDF computed via Algorithm 1 and Eigendecomposition
#'
#' @param x a vector
...
#' @noRd
#' @examples
#' FAStCDF_algo1_eig(c(0,0), log = FALSE )
FAStCDF_algo1_eig = function(x,
                       mu = rep(0, length(x)),
                       Sigma = diag(length(x)),
                       log.p = FALSE,
                       eps = 2,
  ...
  paramsEP = getParamsEP_priorMean_priorVar_algo1_cpp(
    X = X,
    y = rep(1, n),
    priorMean = priorMean,
